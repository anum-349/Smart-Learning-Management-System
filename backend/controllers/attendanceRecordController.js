...
export const getAllData = async (req, res) => {
    try {
        const data = await AssignmentSubmission.find()
            .populate("assignmentId", "title courseAssignmentId dueDate totalMarks")
...
export const getSubmissionsByStudent = async (req, res) => {
    try {
        ...
        const submissions = await AssignmentSubmission.find({ studentId })
            .populate({
                path: "assignmentId",
                select: "title courseAssignmentId dueDate totalMarks"
            });
        res.status(200).json(submissions);
    } catch (err) {
        res.status(500).json({ error: err.message });
...
export const gradeSubmission = async (req, res) => {
    try {
        ...
        const submission = await AssignmentSubmission.findByIdAndUpdate(
            ...
            { marks },
            { new: true }
        ).populate("assignmentId", "title courseAssignmentId totalMarks");
